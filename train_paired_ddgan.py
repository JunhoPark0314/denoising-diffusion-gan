...
def sample_from_model(coeff, generator, n_time, x_init, T, opt, netTrg, real_data):
    x = x_init
    x0_list = []
    with torch.no_grad():
...
def train(rank, gpu, args):
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for iteration, (x, y) in enumerate(data_loader):
            ...
            #D_fake = netD(x_t_D_predict, t, x_tp1_D).view(-1)
    
            # train with real
            D_ = netD(torch.cat([x_t_D,x_t_D_predict]), torch.repeat(t, (2,)), torch.repeat(x_tp1_D, (2,1,1,1)).view(-1)
            
            D_real, D_fake = torch.chunk(D_, 2, 1)
            errD_real = F.softplus(-(D_real - D_fake))
        ...
        if rank == 0:
            ...
            fake_sample = sample_from_model(coeff, netG, (args.num_timesteps // args.step_size), x_t_1, T, args, netTrg, real_data)
