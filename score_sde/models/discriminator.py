...
# ---------------------------------------------------------------
from score_sde.models.modulate_conv import SynthesisGroupConv2d
import torch
...
class DownConvModulateBlock(nn.Module):
    def __init__(
        self,
        in_channel,
        out_channel,
...
class Discriminator_small_modulate(nn.Module):
  """A time-dependent discriminator for small images (CIFAR10, StackMNIST)."""
  ...
  def __init__(self, nc = 3, ngf = 64, t_emb_dim = 128, act=nn.LeakyReLU(0.2)):
    ...
    # Encoding layers where the resolution decreases
    self.start_conv = conv2d(nc,ngf*2,1, padding=0)
    self.conv1 = DownConvModulateBlock(ngf*2, ngf*2, t_emb_dim = t_emb_dim,act=act)
    
    self.conv2 = DownConvModulateBlock(ngf*2, ngf*4,  t_emb_dim = t_emb_dim, downsample=True,act=act)
    
    
    self.conv3 = DownConvModulateBlock(ngf*4, ngf*8,  t_emb_dim = t_emb_dim, downsample=True,act=act)

    
    self.conv4 = DownConvModulateBlock(ngf*8, ngf*8, t_emb_dim = t_emb_dim, downsample=True,act=act)
